...
begin 
    ...
    delete from trigger_table where id = new.id;
    t_id:=(OLD.id);
...
isunifiedstoreproduct
  ...
  jsonb_array_elements( new.data ->'unifiedStore'->'categories')->'subCategory3'->'value',
   ...
   Coalesce(new.data ->'resellerDetails'->'memberOf'->0,'0'),
    ...
    jsonb_array_elements(new.data ->'level3ProductsList')->'shippingDetails',
...
select jsonb_array_elements(data->'level3ProductsList')->'productLevel3Id' from satish_table where id='3tgs' ;
select productlevel3_id from trigger_table where id='3tgs';
    ...
    (
      SELECT jsonb_agg(
        CASE
          ...
          THEN jsonb_set(elem, '{productLevel3ShortTitle}', '"up_productLevel3ShortTitle"')
          ELSE elem
        END
      )
      FROM jsonb_array_elements(data->'level3ProductsList') AS elem
...
update satish_table set data=jsonb_set(data,'{level3ProductsList,productLevel3ShortTitle}','"up_productLevel3ShortTitle"') where level3ProductsList->'productLevel3Id='3yv7' and actiontype='update'; 
...
update satish_table set data=data #- '{jsonb_array_elements(level3ProductsList)->'productLevel3Id'}' where productLevel3Id='40ce';

UPDATE satish_table actiontype='update'
SET data = jsonb_set(
    data, 
    '{level3ProductsList}', 
    (SELECT jsonb_agg(elem)
...
WHERE id = '3xxe';  
 ...
 select jsonb_array_elements(data->'level3ProductsList')->'productLevel3ShortTitle' from satish_table where id='3xxe' ;
 select product->'productLevel3ShortTitle' from trigger_table where id='3xxe';
...
# DELETE FROM trigger_table WHERE productlevel3_id = '"3xxf"';

# update satish_table set actiontype="UPDATE";

# update satish_table set actiontype='update',productLevel3Id='3zrh' where id='3xxe';
