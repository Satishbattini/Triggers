...
isunifiedstoreproduct
 ) values(      OLD.id,new.data-'level3ProductsList',
  new.data->'level3ProductsList',
  ...
  jsonb_array_elements( new.data ->'unifiedStore'->'categories')->'subCategory3'->'value',
   ...
   Coalesce(new.data ->'resellerDetails'->'memberOf'->0,'0'),
    ...
    jsonb_array_elements(jsonb_array_elements(new.data ->'attachments')->'attachments')->'url',
    ...
    SET data = jsonb_set(
        ...
        WHERE elem->>'productLevel3Id' <> (SELECT productLevel3Id FROM satish_table WHERE id = new.id))
    )
    WHERE id = NEW.id;
...
create or replace trigger trigger_name3
  after update
  ON satish_table
  for each row
...
update satish_table set actiontype='delete' , productLevel3Id='40ce' where id='3xxe';
